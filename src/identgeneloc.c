...
fputs("       -L list chromosome ids only to stdout (def=find proximal genes)\n",fl);
  ...
  switch (rpp->rstyle)
    ...
    case IGL_findproximal:
      if (src == NULL)
        ...
        igl_procchrposfile(src,stdout,rpp);
      break;
    }
  exit(0);
  }
